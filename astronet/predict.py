...
def predict(legacy=False):
    # Keras 3 (TF >= 2.16) dropped SavedModel support in load_model().
    # Load the SavedModel's serving signature directly. The signature is
    # kwargs-only and sanitizes '.' in feature names to '_'.
    ...
    for features, identifiers in ds:
      # The serving signature is kwargs-only and sanitizes '.' to '_'
      # in feature names (e.g. 'global_view_0.3' -> 'global_view_0_3').
      feature_kwargs = {k.replace('.', '_'): v for k, v in features.items()}
      out = infer(**feature_kwargs)
